...
if (ffmpegStatic) {
    try {
        ...
        if (!fs.existsSync(ffmpegPath)) {
            const prodPath = path.join(process.cwd(), '.next/server/vendor-chunks/ffmpeg');
            if (fs.existsSync(prodPath)) {
                ffmpegPath = prodPath;
            } else {
                console.error("FFmpeg binary not found at default or production path.");
                throw new Error("FFmpeg binary not found.");
            }
        }
        ffmpeg.setFfmpegPath(ffmpegPath);
    } catch (e) {
        console.error("Error setting ffmpeg path:", e);
    }
}
