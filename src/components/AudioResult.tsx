import { AudioResult as AudioResultType } from '@/lib/types';
  ...
  return (
    <div className="bg-white rounded-xl shadow-md p-4 border border-gray-200 flex flex-col sm:flex-row items-center justify-between gap-4">
      <div className="flex-grow">
        <p className="font-semibold text-gray-800 truncate" title={result.text}>{result.text}</p>
        <p className="text-sm text-gray-500">{result.voice} - {new Date(result.createdAt).toLocaleString()}</p>
      </div>
