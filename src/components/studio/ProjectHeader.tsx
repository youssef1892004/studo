...
import { ArrowLeft, RotateCcw, RotateCw, MoreHorizontal, Clock, Share, Download, Play, LoaderCircle } from 'lucide-react'; // <-- السطر المضاف
...
export default function ProjectHeader({ 
  projectTitle, setProjectTitle, isGenerating, handleGenerate 
}: ProjectHeaderProps) {
    ...
    <header className="flex items-center justify-between px-4 py-2 bg-white border-b border-gray-200 flex-shrink-0">
      ...
      <div className="flex items-center gap-4">
        <button 
          ...
          className="px-4 py-2 bg-gray-800 hover:bg-gray-900 text-white text-sm font-semibold rounded-lg transition-colors disabled:bg-gray-400 flex items-center gap-2"
        >
          {isGenerating ? <LoaderCircle className="w-4 h-4 animate-spin" /> : <Play className="w-4 h-4" />}
          {isGenerating ? 'Generating...' : 'Generate'}
        </button>
        <button className="p-2 hover:bg-gray-100 rounded-lg transition-colors" title="History">
           <Clock className="w-5 h-5 text-gray-600" />
        </button>
        <button className="p-2 hover:bg-gray-100 rounded-lg transition-colors" title="Share">
           <Share className="w-5 h-5 text-gray-600" />
        </button>
        <button className="p-2 bg-gray-100 hover:bg-gray-200 rounded-lg transition-colors" title="Download">
